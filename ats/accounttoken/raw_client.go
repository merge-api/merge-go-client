...
func (r *RawClient) RegenerateCreate(
	ctx context.Context,
	opts ...option.RequestOption,
) (*core.Response[*ats.RegenerateAccountToken], error) {
	options := core.NewRequestOptions(opts...)
	baseURL := internal.ResolveBaseURL(
		options.BaseURL,
		r.baseURL,
	...
	endpointURL := baseURL + "/ats/v1/account-token/regenerate"
	headers := internal.MergeHeaders(
		r.options.ToHeader(),
		options.ToHeader(),
	)
	var response *ats.RegenerateAccountToken
	raw, err := r.caller.Call(
		ctx,
		&internal.CallParams{
			URL:             endpointURL,
	...
	return &core.Response[*ats.RegenerateAccountToken]{
		StatusCode: raw.StatusCode,
		Header:     raw.Header,
		Body:       response,
	}, nil
