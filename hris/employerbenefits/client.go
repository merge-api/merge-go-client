...
func (c *Client) List(
...
) (*core.Page[*hris.EmployerBenefit], error) {
	options := core.NewRequestOptions(opts...)
	...
	readPageResponse := func(response *hris.PaginatedEmployerBenefitList) *internal.PageResponse[*string, *hris.EmployerBenefit] {
		var zeroValue *string
		next := response.Next
		results := response.Results
		return &internal.PageResponse[*string, *hris.EmployerBenefit]{
			Next:    next,
			Results: results,
			Done:    next == zeroValue,
		}
...
func (c *Client) Retrieve(
	...
	endpointURL := internal.EncodeURL(
		baseURL+"/hris/v1/employer-benefits/%v",
		id,
	)
	queryParams, err := internal.QueryValues(request)
	if err != nil {
