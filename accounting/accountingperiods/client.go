...
func (c *Client) List(
...
) (*core.Page[*accounting.AccountingPeriod], error) {
	options := core.NewRequestOptions(opts...)
	...
	readPageResponse := func(response *accounting.PaginatedAccountingPeriodList) *internal.PageResponse[*string, *accounting.AccountingPeriod] {
		var zeroValue *string
		next := response.Next
		results := response.Results
		return &internal.PageResponse[*string, *accounting.AccountingPeriod]{
			Next:    next,
			Results: results,
			Done:    next == zeroValue,
		}
...
func (c *Client) Retrieve(
	...
	endpointURL := internal.EncodeURL(
		baseURL+"/accounting/v1/accounting-periods/%v",
		id,
	)
	queryParams, err := internal.QueryValues(request)
	if err != nil {
