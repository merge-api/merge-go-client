...
func (c *Client) List(
...
) (*core.Page[*accounting.AccountDetailsAndActions], error) {
	options := core.NewRequestOptions(opts...)
	...
	readPageResponse := func(response *accounting.PaginatedAccountDetailsAndActionsList) *internal.PageResponse[*string, *accounting.AccountDetailsAndActions] {
		var zeroValue *string
		next := response.Next
		results := response.Results
		return &internal.PageResponse[*string, *accounting.AccountDetailsAndActions]{
			Next:    next,
			Results: results,
			Done:    next == zeroValue,
		}
