...
func (c *Client) List(ctx context.Context, request *accounting.GeneralLedgerTransactionsListRequest) (*accounting.PaginatedGeneralLedgerTransactionList, error) {
	...
	if request.PostedDateAfter != nil {
		queryParams.Add("posted_date_after", fmt.Sprintf("%v", request.PostedDateAfter.Format(time.RFC3339)))
	}
	if request.PostedDateBefore != nil {
		queryParams.Add("posted_date_before", fmt.Sprintf("%v", request.PostedDateBefore.Format(time.RFC3339)))
	}
	...
	var response *accounting.PaginatedGeneralLedgerTransactionList
...
func (c *Client) Retrieve(ctx context.Context, id string, request *accounting.GeneralLedgerTransactionsRetrieveRequest) (*accounting.GeneralLedgerTransaction, error) {
	baseURL := "https://api.merge.dev/api"
	if c.baseURL != "" {
		baseURL = c.baseURL
	}
	endpointURL := fmt.Sprintf(baseURL+"/"+"accounting/v1/general-ledger-transactions/%v", id)
